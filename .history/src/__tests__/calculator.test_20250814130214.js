import { calculateSolvedValue, formatValue, getMonthlyPayment, getRepaymentYears, getEffectiveAnnualRate } from '../src/calculator';
...
describe('RBF Calculator Functions', () => {
  describe('calculateSolvedValue', () => {
    ...
    test('should calculate loan principle correctly', () => {
      const result = calculateSolvedValue(testValues, 'loanPrinciple');
      expect(result).toBe(5000); // $7500 / 1.5 = $5000
    });

    test('should calculate loan amount correctly', () => {
      const result = calculateSolvedValue(testValues, 'loanAmount');
      expect(result).toBe(7500); // $5000 * 1.5 = $7500
    });

    test('should calculate loan fee correctly', () => {
  ...
  describe('formatValue', () => {
    ...
    test('should format currency values with prefix', () => {
      const result = formatValue('loanPrinciple', 5000);
