...
jest.mock('../src/calculator', () => ({
  ...
  formatValue: jest.fn((key, value) => {
    ...
    return `${prefixes[key] || ''}${value}${suffixes[key] || ''}`;
  }),
...
describe('RBFCalculator App', () => {
  ...
  test('displays all input fields', () => {
    ...
    expect(screen.getByLabelText('Revenue Share Rate')).toBeInTheDocument();
    expect(screen.getByLabelText('Repayment Period')).toBeInTheDocument();
  ...
  test('displays calculated value for the selected variable', () => {
    ...
    const repaymentPeriodInput = screen.getByLabelText('Repayment Period');
    expect(repaymentPeriodInput).toHaveClass('text-2xl font-bold text-gray-800');
    expect(repaymentPeriodInput).toHaveTextContent('82 months');
  });
  ...
  test('displays calculation details', () => {
    ...
    expect(screen.getByText(/Monthly Payment/)).toBeInTheDocument();
    expect(screen.getByText(/Factor Rate/)).toBeInTheDocument();
